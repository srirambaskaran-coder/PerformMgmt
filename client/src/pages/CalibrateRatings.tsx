...
export default function CalibrateRatings() {
  ...
  // Enrich evaluations with location, level, and grade names
  const enrichedEvaluations = useMemo(() => {
    if (!evaluations) return [];

    return (evaluations as any[]).map((evaluation: any) => {
      const location = (locations as any[])?.find(
        (l: any) => l.id === evaluation.locationId
      );
      const level = (levels as any[])?.find(
        (l: any) => l.id === evaluation.levelId
      );
      const grade = (grades as any[])?.find(
        (g: any) => g.id === evaluation.gradeId
      );
  ...
  }, [evaluations, locations, levels, grades]);

  // Filter evaluations
  const filteredEvaluations = useMemo(() => {
    if (!enrichedEvaluations) return [];

    return enrichedEvaluations.filter((evaluation: any) => {
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead data-testid="table-header-department">
                    Department
                  </TableHead>
              ...
              <TableBody>
                {filteredEvaluations.map((evaluation: any) => (
                  ...
                  >
                    ...
                    <TableCell
                      data-testid={`table-department-${evaluation.id}`}
                    >
                      {evaluation.departmentName}
                    </TableCell>
